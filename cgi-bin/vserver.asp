...
Public Sub ASPUpload(operation,page,afilename,bboothumb)
	...
	Select Case operation
		Case "add"
			...
			tmpspec = getroot("folder",pginfo(9))											'//Finds Root Folder from Request URL		//
			response.Write (tmpspec)
			tmpspec = fso.BuildPath(tmpspec,pginfo(9))										'//builds path [drive:]\[root][images path]	//
			...
			End If																			'//END CHECK FILE QUALIFICATIONS			//
					objUpload.AllowedFilesList = "jpg,jpeg,gif,png,bmp,wmf"						'//Allow only image uploads					//
